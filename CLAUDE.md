...
Single sans-serif face **Inter** for both display and body (no more serif/sans pairing), loaded via `next/font/google` in `app/layout.tsx` (alongside JetBrains Mono) and exposed as the `--font-inter` CSS variable, consumed by both `theme.fontFamily.display` and `.sans` in `tailwind.config.js` — they resolve to the same font on purpose. JetBrains Mono (`--font-mono`, `.font-mono`) is used more prominently in v2 for numbers/data/labels, matching the reference set in `inpsiration/v2/`. `.font-display` used to be silently overridden by a hardcoded system-serif rule left in `globals.css`; that's been removed, so the Tailwind utility is the only source of truth for it. Don't reintroduce a manual `.font-display` override in `globals.css`. (This pairing has changed several times: Fraunces → Times New Roman → Georgia/Helvetica Neue → Sora/Inter → Playfair Display/Plus Jakarta Sans → **Inter (single face) + JetBrains Mono**. If display typography comes up again, check `git log -- tailwind.config.js app/layout.tsx` for the current state rather than assuming any of these.)

## Couleurs (v2 — blanc/gris neutre, vert comme seul accent)

The `--mv-*` tokens in `app/globals.css` (light `:root` + `.dark`) moved off the warm Forêt & Crème ivory (2026-08-15, v2 design pass) to a neutral white/grey app shell (`--mv-surface: #ffffff`, `--mv-cream-soft: #f4f4f3`, `--mv-border: #e5e5e0`) with green as the **sole surviving accent color** — `--mv-amber` is deliberately aliased to green (not a distinct hue), red is reserved for destructive/error states only. This is a deliberate, explicit instruction — don't reintroduce a second decorative accent color or a warm/cream tint on the main app shell without checking this section first. `--mv-green: #1E4B33` (light) / `#4a9e6e` (dark) itself is unchanged from the earlier Forêt & Crème pass (still traces back to the sibling Minerva Reach product, `github.com/Endsi3g/minerva-os-lite-desktop`, private). Chart/graph color literals (`components/charts/*`, `app/(dashboard)/overview/page.tsx`, `components/pdf/ProposalDocument.tsx`, `components/audit/ProcessFlowDiagram.tsx`) hardcode hex rather than reference the CSS vars — grep for stray non-green/red hex before adding new hardcoded chart colors.
