...
export default function OnboardingPage() {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex items-center justify-center p-4 sm:p-8 font-sans text-zinc-900">
      {loading && <BonsaiPlantLoader title="Configuration…" subtitle="Création de votre espace de travail" />}

      <div className="bg-white border border-zinc-200/90 rounded-[28px] shadow-sm max-w-[1080px] w-full p-6 sm:p-12 grid grid-cols-1 lg:grid-cols-[1.1fr_1fr] gap-10 items-stretch">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-zinc-400 font-mono">{step}/{STEPS.length}</span>
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <div className="flex items-center gap-5">
                    <button
                      ...
                      className="relative w-20 h-20 rounded-full border-2 border-dashed border-zinc-300 bg-zinc-50 flex items-center justify-center overflow-hidden hover:border-[#059669] transition-colors group shrink-0"
                    >
                      {avatarUrl ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" />
                      ) : (
                        <Plus className="w-6 h-6 text-zinc-400 group-hover:text-[#059669] transition-colors stroke-[1.5]" />
                      )}
                    </button>
                    <div className="space-y-1.5">
                      <p className="text-sm font-bold text-zinc-900">Profile picture</p>
                      <button
                        type="button"
                        onClick={() => fileRef.current?.click()}
                        disabled={uploadingAvatar}
                        className="inline-flex items-center px-4 py-1.5 rounded-lg border border-zinc-200 bg-white hover:bg-zinc-50 text-xs font-bold text-zinc-800 transition-all cursor-pointer"
                      >
                        {uploadingAvatar ? 'Téléchargement…' : 'Upload image'}
                      </button>
                      <p className="text-[11px] text-zinc-400">*.png, *.jpeg files up to 10MB</p>
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Full name</label>
                      <input
                        type="text"
                        value={fullName}
                        onChange={(e) => setFullName(e.target.value)}
                        placeholder="Jane Smith"
                        className="w-full px-3.5 py-2.5 border border-zinc-200 rounded-xl text-sm focus:outline-none focus:border-[#059669] transition-colors"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Bio</label>
                      <textarea
                        value={bio}
                        onChange={(e) => setBio(e.target.value)}
                        rows={2}
                        placeholder="A few words about yourself..."
                        className="w-full px-3.5 py-2.5 border border-zinc-200 rounded-xl text-sm focus:outline-none focus:border-[#059669] resize-none transition-colors"
                      />
                    </div>
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Work email</label>
                      <input
                        type="email"
                        value={userEmail}
                        readOnly
                        className="w-full px-3.5 py-2.5 border border-zinc-200 rounded-xl text-sm text-zinc-500 bg-zinc-50/50 cursor-default"
                      />
                    </div>
                    <div className="flex items-center justify-between pt-1">
                      <div>
                        <p className="text-xs font-bold text-zinc-900">Send me product updates</p>
                        <p className="text-[11px] text-zinc-400">Stay informed about new features and improvements.</p>
                      </div>
              ...
              {step === 2 && (
                ...
                >
                  ...
                  <p className="text-sm text-zinc-500">Aidez-nous à personnaliser votre expérience selon votre fonction.</p>

                  <div className="space-y-3">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Intitulé de poste</label>
                      <input
                        type="text"
                        value={roleTitle}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Département</label>
              ...
              {step === 3 && (
                ...
                >
                  ...
                  <p className="text-sm text-zinc-500">Invitez vos collègues à rejoindre Minerva. Vous pourrez le faire plus tard aussi.</p>

                  <div className="space-y-3">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Taille de l&apos;équipe</label>
                    ...
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Inviter des collègues</label>
                      <textarea
                        value={inviteEmails}
                        ...
                        className="w-full px-3.5 py-2.5 border border-zinc-200 rounded-xl text-sm focus:outline-none focus:border-[#059669] resize-none transition-colors"
                      />
                      <p className="text-[11px] text-zinc-400">Séparez les adresses par des virgules.</p>
                    </div>
                  </div>
                </motion.div>
              )}
              ...
              {step === 4 && (
                ...
                >
                  ...
                  <p className="text-sm text-zinc-500">Configurez votre vue par défaut et vos réglages régionaux.</p>

                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-zinc-900">Page d&apos;accueil par défaut</label>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        <label className="text-xs font-bold text-zinc-900">Fuseau horaire</label>
                        <select
                          value={timezone}
                          onChange={(e) => setTimezone(e.target.value)}
                          className="w-full px-3 py-2.5 border border-zinc-200 rounded-xl text-xs focus:outline-none focus:border-[#059669] bg-white cursor-pointer"
                        >
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs font-bold text-zinc-900">Langue</label>
                        <select
                          value={language}
            ...
            <div className="flex items-center gap-3 pt-4">
              {step > 1 && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 text-sm font-semibold text-zinc-700 hover:bg-zinc-50 transition-all cursor-pointer"
                >
        ...
        <div className="hidden lg:flex flex-col border border-zinc-200/80 rounded-2xl bg-zinc-50/50 overflow-hidden shadow-sm">
          <div className="grid grid-cols-[180px_1fr] flex-1 h-full min-h-[460px]">
            {/* Mini Sidebar */}
            <div className="border-r border-zinc-200/70 p-4 space-y-3 bg-zinc-50/80">
              <div className="flex items-center justify-between gap-2 pb-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div className="w-6 h-6 rounded-md bg-[#059669] flex items-center justify-center shrink-0" />
                  <span className="text-xs font-bold text-zinc-900 truncate">Workspace</span>
                </div>
                <span className="text-zinc-400 text-xs">‹</span>
              </div>
              <div className="space-y-1.5 pt-1">
                {[true, false, false, false, false, false, false, false, false].map((active, i) => (
                  <div key={i} className={`h-7 rounded-lg border ${active ? 'bg-[#059669]/10 border-[#059669]/20' : 'bg-zinc-100/60 border-zinc-200/50'}`} />
                ))}
              </div>
            </div>
            ...
            <div className="p-4 flex flex-col justify-between space-y-3 bg-white">
              <div className="space-y-3">
                <div className="flex items-center gap-2">
                  <div className="flex-1 h-7 rounded-lg bg-zinc-50 border border-zinc-200" />
                  <div className="w-7 h-7 rounded-lg bg-zinc-50 border border-zinc-200" />
                  <div className="w-7 h-7 rounded-lg bg-zinc-50 border border-zinc-200" />
                </div>
                <div className="border border-zinc-200 rounded-xl overflow-hidden divide-y divide-zinc-200">
                  {Array.from({ length: 7 }).map((_, i) => (
                    <div key={i} className={`grid grid-cols-4 divide-x divide-zinc-200 h-8 ${i === 0 ? 'bg-zinc-50' : ''}`}>
                      {Array.from({ length: 4 }).map((_, j) => (
                        <div key={j} className={`${i === 0 ? 'bg-zinc-100/50' : 'bg-white'}`} />
                      ))}
                    </div>
                  ))}
                </div>
              ...
              <div className="flex items-center gap-1.5">
                {Array.from({ length: 5 }).map((_, i) => (
                  <div key={i} className={`w-6 h-6 rounded-md border ${i === 0 ? 'bg-[#059669] border-[#059669]' : 'bg-zinc-50 border-zinc-200'}`} />
                ))}
              </div>
            </div>
          </div>
